...
io.sockets.on('connection', function(socket){
    ...
    socket.broadcast.emit("FEEDBACK","[+] ROBOT USER" + user_count + "CONNECTED");
    user_count ++;
    ...
    socket.on('USBCONNECT',function(data){
        ...
        if(portName.length > 0){
                ...
                serialPort = new SerialPort(data, { //test port
                    baudrate: 9600,
                    dataBits: 8,
                    parity: 'none',
                    stopBits: 1,
                ...
                //open port
                serialPort.on("open", function () { //"data" ---> read data from extern hardware
